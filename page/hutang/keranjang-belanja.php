<?php
    ...
    switch($aksi) {	
        ...
        if(!empty($_SESSION["cart_item_hutang"])) {
            if(in_array($productByCode['kode_produk'],array_keys($_SESSION["cart_item_hutang"]))) {
                foreach($_SESSION["cart_item_hutang"] as $k => $v) {
                        if($productByCode['kode_produk'] == $k) {
                            if(empty($_SESSION["cart_item_hutang"][$k]["jumlah_beli"])) {
                                $_SESSION["cart_item_hutang"][$k]["jumlah_beli"] = 0;
                            }
                            if ( $_SESSION["cart_item_hutang"][$k]["jumlah_beli"]+$jumlah_beli<=$stok_produk){
                                $_SESSION["cart_item_hutang"][$k]["jumlah_beli"] += $jumlah_beli;
                            }else {
                                echo "<script> alert('Jumlah beli tidak boleh melebihi stok produk');</script>";
                                $_SESSION["cart_item_hutang"][$k]["jumlah_beli"];
                            }
                            
                        }
                }
            } else {
                $_SESSION["cart_item_hutang"] = array_merge($_SESSION["cart_item_hutang"],$itemArrayHutang);
            }
        } else {
            $_SESSION["cart_item_hutang"] = $itemArrayHutang;
        }
        break;
        case "hapus_item":
    		...
    		if(!empty($_SESSION["cart_item_hutang"])) {
                foreach($_SESSION["cart_item_hutang"] as $k => $v) {
                        if($_POST["kode_produk"] == $k)
                            unset($_SESSION["cart_item_hutang"][$k]);
                        if(empty($_SESSION["cart_item_hutang"]))
...
<?php
$tot=0;	if(!empty($_SESSION["cart_item_hutang"])) {
    foreach ($_SESSION["cart_item_hutang"] as $item){
    ...
    <?php }} ?>
            ...
            <tr>
                ...
                <td colspan="2">
                    <h4>
                        <input name="diskon" id="diskon" type="number" class="form-control" required>
                    </h4>
                </td>
            </tr>
            <tr>
...
<div class="row">
    <div class="col-sm-7">
        ...
        <div class="form-group">
            ...
            <input name="uang_muka" id="uang_muka"  type="number" class="form-control" required>
        </div>
        <div class="form-group">
            <label>Cicilan Bulan:</label>
            <input name="cicilan_bulan" id="cicilan_bulan"  type="number" class="form-control" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" id="tampil_kekurangan" class="form-control" disabled>
